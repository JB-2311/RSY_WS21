...
function [Position_sicher, Position_ueb]=Uebergabeposition(position_YB2, uebergabehoehe, abstand, Rolle)
    ...
    % Definition der Übergabeposition & Orientierung des Greifers je nach Rolle
    ...
    % Berechnung der Sicherheitsposition (x_s y_s z_ueb) horizontal verschoben vor der Übergabeposition 
    r_ueb_sicher=r_ueb-abstand;
    x_s=cos(theta1)*(r_ueb_sicher/2);
    y_s=sin(theta1)*(r_ueb_sicher/2);
    
    % Definition der Sicherheitsposition & Orientierung des Greifers je nach Rolle
    if Rolle == 1 %Master-Rolle
